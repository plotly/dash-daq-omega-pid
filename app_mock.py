...
def command_string(start_button, stop_button, reset_button, autotune_button, manual_button):
    master_command = {"START": start_button,
                      "STOP": stop_button, "RESET": reset_button, "AUTO": autotune_button, "MANUAL":manual_button}
...
def set_point(set_pid_button, autotune_button, PID_setpoint, PID_setpoint_auto, PID_man_auto):
    if PID_man_auto:
...
@app.callback(
    ...
    State("thermo-type", "value"),
    State("filter-rate", "value"),
    State("autotune-timestamp", "children"),
    State("autotune-timeout", "value"),
    ]
)
def serial_monitor(intervals, autotune_button, command, max_rate, dev_gain, pro_gain, int_gain, couple_type, filter_rate, autotune_time, autotune_timeout):
...
def data_set(temperature, command, start, start_button, PID_setpoint, dev_gain, pro_gain, int_gain):
    if command == "START":
...
def PID_percent(n_intervals, command, start, PID_setpoint, dev_gain, pro_gain, int_gain):
    if command == "START":
